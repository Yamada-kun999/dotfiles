...
DIR="$HOME/Images"
ROFI_THEME="~/.config/rofi.dude/wallpapers.rasi"
...
if [ "$1" = "" ]; then
	file=$(echo -en "$cmd" | rofi -dmenu -i -p "Witch One" -theme "$ROFI_THEME")
else
	if [ $1 = "rand" ]; then
...
swww img --transition-type any --transition-fps 60  "$file" > /dev/null
...
rofiTheme=$HOME/.config/rofi/wal.rasi
