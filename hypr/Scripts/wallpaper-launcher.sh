...
#	Info    - This script runs the rofi launcher, to select the wallpapers included in the theme you are in.

# Set some variables
theme="animegirls"
wall_dir="${HOME}/Wallpapers/$theme/"
cacheDir="${HOME}/.cache/rofi_walls/${theme}"
...
for imagen in "$wall_dir"/*.{gif,jpg,jpeg,png,webp}; do
	if [ -f "$imagen" ]; then
...
swww img "${wall_dir}/${wall_selection}" --transition-duration ${SWWW_TRANSITION_DURATION} -t ${SWWW_TRANSITION}
